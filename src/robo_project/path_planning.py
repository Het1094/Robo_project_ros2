...
# ================================================================
MAP_SOURCE = "simulation"        # "real" | "simulation" | "test"

MAP_FILE = "~/robo_project_ws/src/environment/Denmark.glb"            # used when MAP_SOURCE = "test"
REAL_MAP_IMAGE_PATH = "~/robo_project_ws/src/resources/house_map.png"   # used when MAP_SOURCE = "real"
REAL_MAP_THRESHOLD  = 127             # pixels darker than this = wall
GLB_SCENE_PATH = "~/robo_project_ws/src/environment/Denmark.glb"  # MAP_SOURCE = "simulation"
GLB_MAP_RESOLUTION = 1024
...
# habitat_sim is ONLY needed for MAP_SOURCE="simulation" (.glb testing).
# Real robot deployment (MAP_SOURCE="real") never needs this import.
try:
    import habitat_sim
...
# ================================================================
# ── MAP LOADERS — real hand-drawn map OR .glb simulation ─────────
# Grid convention everywhere: 1 = free, 0 = obstacle (astar.py)
# ================================================================

def load_real_map(path, threshold=REAL_MAP_THRESHOLD):
    """
    Load a real-world map: hand-drawn/scanned photo, or pre-made
    binary grid. This is what the real robot actually uses —
    no simulator, no 3D scene file needed.
    """
    if path.lower().endswith('.npy'):
        ...
        if raw.max() > 1:
            raw = raw / raw.max()
        grid = np.round(raw).astype(np.uint8)
        print(f"Loaded binary grid from: {path}")
    else:
        img = cv2.imread(path, cv2.IMREAD_GRAYSCALE)
        if img is None:
            raise FileNotFoundError(f"Could not read map image: {path}")
        # Dark pixels (drawn lines/walls) = obstacle = 0
        grid = np.where(img < threshold, 0, 1).astype(np.uint8)
        print(f"Loaded hand-drawn map from: {path}")
        print(f"  Image size: {img.shape[1]}x{img.shape[0]} px")

    print(f"  Grid size : {grid.shape[1]}x{grid.shape[0]} cells")
    print(f"  Free      : {np.sum(grid==1)} cells")
    print(f"  Obstacle  : {np.sum(grid==0)} cells")
    return grid


def load_glb_map(scene_path, map_resolution=GLB_MAP_RESOLUTION):
    """
    Load a map from a Habitat .glb 3D scene — SIMULATION/TESTING ONLY.
    Uses habitat_sim's pathfinder to generate a top-down occupancy grid.
    Real robot deployment never calls this — only used to benchmark
    against the CMN paper's Habitat-based evaluation.
    """
    if not HABITAT_SIM_AVAILABLE:
        raise ImportError(
            "habitat_sim is not installed. MAP_SOURCE='simulation' "
            "needs it for .glb scenes. Use MAP_SOURCE='real' instead."
        )
    ...
    if not sim.pathfinder.is_loaded:
        sim.close()
        raise RuntimeError("Navmesh/pathfinder failed to load for scene.")
    ...
    scene_size = max(upper[0]-lower[0], upper[2]-lower[2])
    meters_per_pixel = scene_size / map_resolution

    topdown_bool = sim.pathfinder.get_topdown_view(
        meters_per_pixel=meters_per_pixel, height=height)
    sim.close()

    # topdown_bool: True = navigable (free) → grid: 1=free, 0=obstacle
    grid = np.where(topdown_bool, 1, 0).astype(np.uint8)
    print(f"Loaded .glb scene: {scene_path}")
    print(f"  Grid size : {grid.shape[1]}x{grid.shape[0]} cells")
    print(f"  Free      : {np.sum(grid==1)} cells")
    print(f"  Obstacle  : {np.sum(grid==0)} cells")
    return grid
...
def load_map(fname=None):
    """Loads the map according to MAP_SOURCE: 'real' | 'simulation' | 'test'."""
    if MAP_SOURCE == "real":
        import os
        if not os.path.exists(REAL_MAP_IMAGE_PATH):
            print(f"\nERROR: Real map file not found: {REAL_MAP_IMAGE_PATH}")
            print(f"  Set REAL_MAP_IMAGE_PATH to your actual map file,")
            print(f"  or set MAP_SOURCE = 'test' to use synthetic maps.")
            raise FileNotFoundError(REAL_MAP_IMAGE_PATH)
        grid = load_real_map(REAL_MAP_IMAGE_PATH, REAL_MAP_THRESHOLD)
        print(f"Map loaded     : {grid.shape[0]} x {grid.shape[1]}  (real map)")
        return grid

    if MAP_SOURCE == "simulation":
        grid = load_glb_map(GLB_SCENE_PATH, GLB_MAP_RESOLUTION)
        print(f"Map loaded     : {grid.shape[0]} x {grid.shape[1]}  (simulation)")
        return grid

    # MAP_SOURCE == "test"
    raw  = np.load(fname or MAP_FILE)
    raw  = np.array(raw, dtype=float)
    if raw.max() > 1:
        raw = raw / raw.max()
    grid = np.round(raw).astype(int)
    print(f"Map loaded     : {grid.shape[0]} x {grid.shape[1]}  (test map)")
...
def compute_heuristic_vector(grid, goal):
    ...
    This is the Global Value Field from the CMN paper.
    """
    print("\n--- Heuristic Vector ---")
    H, W = grid.shape
    ...
    gr, gc = goal
    if grid[gr, gc] == 0:
        print(f"WARNING: Goal {goal} is inside an obstacle! Snapping...")
        free = np.argwhere(grid == 1)
        dists = np.sqrt((free[:,0]-gr)**2 + (free[:,1]-gc)**2)
        goal = tuple(free[np.argmin(dists)])
        print(f"  Snapped to: {goal}")
...
def main():
    ...
    if MAP_SOURCE == "real":
        print(f"Map file     : {REAL_MAP_IMAGE_PATH}")
    elif MAP_SOURCE == "simulation":
        print(f"Scene        : {GLB_SCENE_PATH}")
    else:
